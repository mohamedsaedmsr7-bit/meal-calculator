...
function MealCard({ index, onRemove, showRemove }) {
  ...
  const filtered = useMemo(() => {
    ...
    return catFoods.filter(f =>
      f.ar.includes(search) || f.en.toLowerCase().includes(q)
    );
  }, [search, catFoods]);

  // ===== المنطق الحسابي الأساسي =====
  const totalCals = useMemo(() => {
    const g = parseFloat(grams);
    if (!g || g <= 0) return 0;
    return (food.cal * g) / 100;
  }, [food, grams]);
  ...
  const alternatives = useMemo(() => {
    ...
    return catFoods
      ...
      .map(f => ({
        ...f,
        needed: Math.round((totalCals / f.cal) * 100),
      }))
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: C.neon, fontWeight: 700, opacity: 0.6,
      ...
      <div style={{
        ...
        {filtered.length === 0 && (
          <p style={{ color: C.subtext, fontSize: "0.8rem", textAlign: "center", padding: 12 }}>
            لا توجد نتائج 🔎
          </p>
        )}
        {filtered.map(f => {
          ...
          return (
            <button
              ...
              <span style={{
                background: `${stateTag(f.state)}18`,
                color: stateTag(f.state),
                padding: "1px 8px", borderRadius: 6,
                fontSize: "0.65rem", fontWeight: 600,
              }}>{f.state}</span>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 1 }}>
                <span style={{ color: active ? C.neon : C.text, fontSize: "0.85rem", fontWeight: active ? 700 : 400 }}>
                  {f.ar}
                </span>
      ...
      {!grams && food && (
        <div style={{
          ...
          color: C.muted, fontSize: "0.78rem", lineHeight: 1.6,
        }}>
...
// ========== كومبوننت النتائج ==========
function ResultsPanel({ totalCals, food, grams, alternatives }) {
  return (
    <div>
      {/* بطاقة إجمالي السعرات */}
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ color: C.subtext, fontSize: "0.7rem", marginBottom: 2 }}>
              إجمالي السعرات
            </div>
            <div style={{ color: C.neon, fontWeight: 800, fontSize: "1.5rem", lineHeight: 1 }}>
              {Math.round(totalCals)}
              <span style={{ fontSize: "0.75rem", fontWeight: 400, marginRight: 4, color: C.subtext }}>
                kcal
              </span>
            </div>
          </div>
      ...
      {/* قائمة البدائل */}
      <div style={{
        fontSize: "0.7rem", color: C.teal, fontWeight: 700,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        {alternatives.map(alt => (
          <AlternativeRow key={alt.en} alt={alt} totalCals={totalCals} />
        ))}
      </div>
    </div>
  );
...
// ========== صف بديل واحد ==========
function AlternativeRow({ alt, totalCals }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
        <div style={{ textAlign: "left" }}>
          <span style={{ color: C.teal, fontWeight: 800, fontSize: "1.1rem" }}>
            {alt.needed}
          </span>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <span style={{
            background: `${stateTag(alt.state)}18`,
            color: stateTag(alt.state),
...
export default function App() {
  ...
  const addMeal = () => {
    if (meals.length < 4)
      setMeals(prev => [...prev, { id: Date.now() }]);
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: `
          radial-gradient(ellipse 60% 40% at 80% 10%, ${C.neon}07 0%, transparent 60%),
      ...
      <div style={{
        ...
        <header style={{ textAlign: "center", padding: "2.2rem 0 1.6rem" }}>
          <div style={{
            ...
            <span>⚡</span> Smart Nutrition Calculator
          </div>
          <h1 style={{
            margin: "0 0 8px",
        ...
        <div style={{
          ...
          gap: 6, marginBottom: "1.6rem",
          flexWrap: "wrap",
        }}>
          {["اختر الفئة", "اختر الصنف", "أدخل الكمية", "شاهد البدائل"].map((s, i) => (
            <div key={i} style={{
        ...
        {meals.length === 4 && (
          <p style={{ textAlign: "center", color: C.muted, fontSize: "0.72rem", marginTop: 10 }}>
            الحد الأقصى 4 وجبات للمقارنة
          </p>
        )}
        ...
        <footer style={{
          ...
          color: C.muted, fontSize: "0.68rem", lineHeight: 1.8,
        }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        input:focus {
          border-color: ${C.neon}66 !important;
          box-shadow: 0 0 0 3px ${C.neon}12;
        }
